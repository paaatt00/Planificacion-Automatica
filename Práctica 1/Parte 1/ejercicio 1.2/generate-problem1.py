...
def main():
    ...
    parser.add_option('-r', '--carriers', metavar='NUM', dest='carriers', type=int, help='the number of carries')
    parser.add_option('-l', '--locations', metavar='NUM', type=int, dest='locations', help='the number of locations apart from the warehouse ')
    parser.add_option('-h', '--humans', metavar='NUM', type=int, dest='humans', help='the number of humans')
    parser.add_option('-b', '--boxes', metavar='NUM', type=int, dest='boxes', help='the number of boxes available')
    ...
    problem_name = "drone-problem1" + str(options.drones) + "_r" + str(options.carriers) + \
    ...
    with open(problem_name + ".pddl", 'w') as f:
        ...
        for x in human:
            rand1 = random.randint(0, len(location) - 1)
            rand2 = random.randint(0, len(content_types) - 1)
            f.write("\t(human-at " + x + " " + location[rand1] + ")\n")
            f.write("\t(human-needs " + x + " " + content_types[rand2] + ")\n")
